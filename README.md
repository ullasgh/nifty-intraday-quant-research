...
**Cost is size-dependent and this is the single biggest lever on profitability.** NSE intraday (MIS) round-trip costs vary sharply with position notional. However, the benefit of raising capital is NOT uniform across all sizes. Here is the critical crossover:

    Per-name clip    Round-trip cost
    Rs 10,000        10.6245 bps
    Rs 66,667        10.6245 bps   <- crossover point
    Rs 70,000        10.2874 bps
    Rs 93,458        8.5949 bps
    Rs 1,00,000      8.26452 bps
    Rs 10,00,000     4.01652 bps
    Rs 1,00,00,000   3.59172 bps

**IMPORTANT:** At the crossover point (Rs 66,667 per-name clip), the cost is flat at 10.6245 bps. NSE brokerage is `min(Rs 20 flat, 0.03% of notional)`. BELOW the crossover the PERCENTAGE term is the smaller of the two, so brokerage is purely proportional to notional and round-trip bps is therefore CONSTANT. ABOVE it the Rs 20 FLAT cap binds, so brokerage stops growing with size and round-trip bps FALLS as capital rises. At the documented default capital of Rs 1,000,000, your per-name clip is Rs 9,346 (calculated above), which is BELOW the crossover. Therefore you see NO cost reduction from raising capital until your per-name clip exceeds Rs 66,667 (which requires holding capital > Rs 7,134,381 with typical portfolio holdings of 107 names). This is the NSE brokerage structure, not a bug; costs remain 10.6245 bps for all clips from Rs 10,000 to Rs 66,667.

The flow is: capital -> per-name clip size = capital / typical number held -> round-trip cost at that clip -> total cost per session = round-trip cost * mean daily turnover. Cost is charged only on TURNOVER, not on notional -- a book that does not trade pays nothing.
...
The command prints a table with one row per year plus an ALL row (total across all years):
    ...
    Tilt backtest  mild / smoothing 0.10 / capital Rs 1,000,000
    Universe all_equity (149 names)   2024-01-01 .. 2025-07-31   388 sessions
    Clip per name Rs 9,346   round-trip 10.62 bps   breakeven turnover 0.43

    year   sessions   gross_bps   turnover   cost_bps   net_bps   ann_net%
    2024        242            5.22     0.1264      1.34     3.88     9.78
    2025        146            3.48     0.1286      1.37     2.11     5.32
    ------------------------------------------------------------------
    ALL         388            4.57     0.1272      1.35     3.22     8.10
...
Note on clip: The "Clip per name" is the capital divided by the number of names HELD, not by the 149-name universe. At Rs 1,000,000 capital, the book typically holds about 107 names (roughly 72% of the universe), so the per-name clip is approximately Rs 1,000,000 / 107 = Rs 9,346. This explains why the clip shown is Rs 9,346, not Rs 6,711 (1,000,000 / 149).
...
- NSE intraday (MIS) round-trip cost is **8.3 bps** at Rs 1,00,000 notional and **4.0 bps** at Rs 10,00,000 notional (`NSEIntradayEquityCosts.round_trip_bps`); market impact dominates above that size, which `SqrtImpactSlippage` / `breakeven_cost_bps` account for separately from these flat/percentage charges.
- A real end-to-end `nq backtest` run (`volume_breakout`, 8 symbols, full year 2024, real NSE costs) confirms the pipeline works but does **not** show a working strategy: gross Sharpe **-0.048**, net Sharpe **-0.233**, ~Rs 541k in total costs, 21,708 trades, and **73.7%** of desired notional unfilled. Treat this as proof the plumbing (data, costs, fills, reporting) is verified, not as evidence `volume_breakout` is profitable -- the honest next step is fixing its sizing/churn (21.7k trades on 8 symbols in a year is very high turnover for the signal counts above), not trusting the current numbers.
